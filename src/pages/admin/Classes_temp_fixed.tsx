...
import { BookOpen, Search, Plus, Users, Clock, Trash2 } from 'lucide-react';
...
export default function Classes() {
  ...
  const [showAddStudent, setShowAddStudent] = useState<{ classId: string; isOpen: boolean }>({ classId: '', isOpen: false });
  const [studentName, setStudentName] = useState<string>('');

  const handleAddStudent = (classId: string) => {
    const classToUpdate = mockClasses.find(c => c.id === classId);
    if (classToUpdate && studentName) {
      classToUpdate.students.push(studentName);
      setStudentName(''); // Reset input field
      setShowAddStudent({ classId: '', isOpen: false }); // Close modal
    }
  };
  ...
  return (
    <div className="p-8">
              ...
              <div key={classItem.id} className="p-6">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="flex gap-2">
                    <button className="text-blue-600 hover:text-blue-800" onClick={() => setShowAddStudent({ classId: classItem.id, isOpen: true })}>
                      <Plus size={18} />
                    </button>
      ...
      {/* Add Student Modal */}
      {showAddStudent.isOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            <h2 className="text-2xl font-bold mb-4">Add New Student</h2>
            <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); handleAddStudent(showAddStudent.classId); }}>
              <div>
              ...
              <div className="flex justify-end gap-2 mt-6">
                <button
                  type="button"
                  onClick={() => setShowAddStudent({ classId: '', isOpen: false })}
