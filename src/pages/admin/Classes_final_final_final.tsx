  ...
  return (
    <div key={grade} className="bg-white rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
      <div className="flex justify-between items-center p-4">
        <h2 className="text-2xl font-bold text-gray-900">{grade} ({classItems.length} classes, {classItems.reduce((total, item) => total + item.students.length, 0)} students)</h2>
        <button
      ...
      {showUploaderForGrade ? (
        ...
        <>
          {classItems.map((classItem) => (
            <div key={classItem.id} className={`p-6 ${classItem.id === currentUserClassId ? 'bg-yellow-100 border border-yellow-400 rounded-lg' : ''}`}>
